...
When modifying Python code in this ERP integration, always run these checks before presenting your work:

1. **Syntax check**: `python -m py_compile <modified-file>` on each changed file.
2. **Test suite**: `pytest <relevant-test-file> -x` for the narrowest test that covers the changed behavior.
3. **Static analysis**: If available, run `mypy <modified-file>` or `ruff check <modified-file>` to catch type and style errors.
4. **Import verification**: After adding new modules, run `python -c "import <module>"` to confirm imports resolve correctly.
...
| Tests (scoped) | `pytest tests/test_order_orchestrator.py -x` |
| Tests (full) | `pytest` |
| Format check | `ruff format --check path/to/file.py` |
