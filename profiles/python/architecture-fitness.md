...
1. **Controllers / Routes** (e.g., `routes/`, `api/`, or `views/`) — Handle HTTP request/response only. NEVER put business logic here.
...
5. **Mappers / Serializers** (e.g., `mappers/`, `serializers/`) — Transform between local domain models and marketplace payloads.
6. **Workers / Tasks** (e.g., `tasks/`, `workers/`) — Async work via Celery, RQ, or similar. Always dispatch through the orchestrator, not directly from routes.
7. **Models** (e.g., `models/`) — Domain entities (SQLAlchemy, Django ORM, etc.). NEVER call external APIs from models.
...
- Adding marketplace-specific `if/elif` statements in an orchestrator → use the factory pattern
- Putting business logic in a Celery task → delegate to an orchestrator
...
1. **Logging**: Use structured logging with marketplace context: `logger.info("message", extra={"marketplace": marketplace, "store_id": store_id, "order_id": order_id})`.
2. **Error handling**: Catch marketplace API exceptions at the service level. Log the full response. Re-raise as domain exceptions for the orchestrator.
3. **Task failures**: All async tasks MUST implement retry policies (`max_retries`, `default_retry_delay`) and dead-letter visibility.
...
- **Async tasks**: Long-running marketplace API calls MUST be dispatched to the task queue, not executed synchronously in HTTP requests.
